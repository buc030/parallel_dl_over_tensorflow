...
class ExternalOptimizerInterface(object):
  ...
  def __init__(self, loss, var_list=None, equalities=None, inequalities=None,
    ...
    print('self.optimizer_kwargs = ' + str(self.optimizer_kwargs))
  ...
  def minimize(self, session=None, feed_dicts=None, fetches=None,
    ...
    if override_loss_grad_func is None:
        loss_grad_func = self._make_eval_func(
            [self._loss, self._packed_loss_grad],
            session, feed_dicts, fetches, loss_callback)
    else:
    ...
    equality_grad_funcs = self._make_eval_funcs(
        self._packed_equality_grads, session, feed_dicts, fetches)

    # Construct inequality constraint functions and associated gradients.
    inequality_funcs = self._make_eval_funcs(
        self._inequalities, session, feed_dicts, fetches)
    inequality_grad_funcs = self._make_eval_funcs(
        self._packed_inequality_grads, session, feed_dicts, fetches)
  ...
  def _make_eval_func(self, tensors, session, feed_dicts, fetches,
                      callback=None):
    ...
    def eval_func(x):
      ...
      augmented_feed_dict.update(self.additional_feed_dict)
      res = avarge_on_feed_dicts(sess=session, target_ops=augmented_fetches,
                                 feed_dicts=feed_dicts, additional_feed_dict=augmented_feed_dict)
  ...
  def _make_eval_funcs(self, tensors, session, feed_dicts, fetches,
                       callback=None):
    return [
        self._make_eval_func(tensor, session, feed_dicts, fetches, callback)
...
class ScipyOptimizerInterface(ExternalOptimizerInterface):
  ...
  def _minimize(self, initial_val, loss_grad_func, equality_funcs,
    ...
    if 'method' in optimizer_kwargs:
        method = optimizer_kwargs['method']
