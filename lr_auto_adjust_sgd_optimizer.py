...
class SgdAdjustOptimizer(optimizer.Optimizer):
    ...
    def update_lr(self):
        with tf.name_scope('update_lr'):
            ...
            with tf.control_dependencies([shift_lr]):
                # 2. update lr
                _update_lr = tf.assign(self.curr_lr, self.lr * (1.0 + self.lr_update_multiplier))
            return tf.group(*[shift_lr, _update_lr])
    ...
    def _prepare(self):
        ...
        with tf.name_scope('handle_snapshots'):
            self.update_snapshot_cond = tf.cond(pred=tf.equal(self.lr_slide_index, 0), true_fn=lambda: tf.group(*(self.shift_snapshots + self.take_snapshot_curr)), false_fn=lambda: tf.no_op())
